...
var Role = (function(_super){
    ...
    _proto.bulletEnter = function(bulletFlyMode, bulletFlyMode1Aim,bulletFlyMode1Speed,bulletSpeed,bulletForce,bulletType,bulletHealingType,bulletDamageType,bulletDamage,bulletCollisionTime,
                                  bulletDeleteType,bulletDeleteType1damage,bulletDeleteType1range,
                                  bulletDeleteType2bulletType,bulletDeleteType2Range,bulletDeleteType2Number,bulletDeleteType2Time){
    ...
    _proto.bulletFly = function(timeBetween){
        ...
        if (this.bulletFlyMode == 1){
            if (this.bulletFlyMode1Aim.visible){
    ...
    _proto.bulletDamage = function(role){
        if (this.bulletDamageType==0){
            ...
            this.actDamage(role,this.bulletDamage);
    ...
    _proto.bulletDelete = function(roleBox){
        this.visible = false;
        if (bulletDeleteType == 1){
            actDamageRange(roleBox,this,this.bulletDeleteType1range,this.bulletDeleteType1damage);
        }
        if (bulletDeleteType == 2 && this.bulletDeleteType2Time > 0){
            var throwTo = Array()
            ...
            while (throwTo.length > this.bulletDeleteType2Number){
                ...
                throwTo = throwTo.splice(ra,1);
            }
            for (var i = throwTo.length-1;i>-1;i--){
                var role1 = throwTo.getChildAt(i);
                var bullet = Laya.Pool.getItemByClass("role",Role);
                bullet.init("bullet1",this.camp,10,0,1,1);
                bullet.bulletEnter(1,role1,1,{x:0,y:0},{x:0,y:0},0,0,0,1,1,2,0,0,2,this.bulletDeleteType2Range,this.bulletDeleteType2Number,this.bulletDeleteType2Time-1);
